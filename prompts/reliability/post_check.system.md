...
- **列表内嵌详情提交** (`submit_context=list_embed`, 如 tasklist 同页展示详情): 点击「提交任务/提交」后 **URL 通常不变**; 成功标志是 **DOM 实体切换** 或 **空态文案**(暂无题目/请去任务中心领题)。**禁止**仅因 URL 未变、或「提交任务」按钮短暂仍存在就判 step_ok=false; 须优先参考【提交后结构化事实】。
